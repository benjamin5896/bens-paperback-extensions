...
var _Sources = (() => {
  ...
  var KaliScan_exports = {};
  __export(KaliScan_exports, {
    KaliScan: () => KaliScan,
    KaliScanInfo: () => KaliScanInfo
  });
  ...
  var BuddyComplexParser = class {
    constructor() {
      ...
      this.isLastPage = ($) => {
        ...
        const pages = $("a.link", "div.paginator").toArray()
          .map((p) => Number($(p).text().trim()))
          .filter((p) => !isNaN(p));
    ...
    parseMangaDetails($, mangaId) {
      ...
      const authors = [];
      for (const authorRaw of $(
        "p:contains(Authors) > a",
        "div.detail"
      ).toArray()) {
      ...
      const description = this.decodeHTMLEntity(
        $("div.section-body > p.content").text().trim()
      );
      const arrayTags = [];
      ...
      const rawStatus = $("p:contains(Status) > a", "div.detail").first().text().trim().toUpperCase();
      const status = rawStatus === "COMPLETED" ? "Completed" : "Ongoing";
    ...
    parseChapterList($, mangaId) {
      ...
      for (const chapter of $("li", "ul.chapter-list").toArray()) {
        ...
        const date = this.parseDate(
          $("time.chapter-update", chapter)?.text() ?? ""
        );
        if (!id) continue;
        const byChapter = id.match(/(?:chapter|ch)-((\d+)(?:[-.]\d+)?)/);
        const byNumber = (id.split("-").pop() ?? "").match(/(\d+)(?:[-.]\d+)?/);
        const rawNum = (byChapter?.[1] ?? byNumber?.[0] ?? "0").replace("-", ".");
        const chapterNumber = isNaN(Number(rawNum)) ? 0 : Number(rawNum);
        chapters.push({
          id,
          name: title,
    ...
    parseChapterDetails($, mangaId, chapterId) {
      const pages = [];
      const imageRegex = /chapImages\s*=\s*["'](.+)["']/gm.exec($.html());
      const chapterImages = imageRegex?.[1]?.split(",");
      if (chapterImages?.length) {
        for (const imageUrl of chapterImages) pages.push(imageUrl.trim());
      } else {
    ...
    parseTags($) {
      ...
      for (const tag of $(
        "li",
        $("a:contains(GENRES)", "ul.header__links-list").next()
      ).toArray()) {
        const label = $(tag).text().replace(",", "").trim();
    ...
    parseHomeSections($, sections, sectionCallback) {
      for (const section of sections) {
        ...
        const topSectionMap = { top_today: 0, top_weekly: 1, top_monthly: 2 };
        if (section.id in topSectionMap) {
          const items = [];
          for (const manga of $("div.top-item", $("div.tab-panel").get(topSectionMap[section.id])).toArray()) {
            const id = this.idCleaner($("a", manga).attr("href") ?? "");
    ...
    getImageSrc(imageObj) {
      ...
      let image;
      if (dataLazy && !dataLazy.startsWith("data")) {
        image = dataLazy;
  ...
  var BuddyComplex = class {
    ...
    async getSearchResults(query, metadata) {
      ...
      const url = new URLBuilder(this.baseUrl).addPathComponent("search").addQueryParameter("page", page).addQueryParameter("q", encodeURI(query?.title || "")).build() + query.includedTags?.map((x) => `&genre%5B%5D=${x.id}`).join("");
      const request = App.createRequest({
        url,
        method: "GET"
      });
    ...
    async getHomePageSections(sectionCallback) {
      ...
      const section5 = App.createHomeSection({
        id: "top_monthly",
        title: "Top Monthly",
        type: import_types.HomeSectionType.singleRowNormal,
        containsMoreItems: true
...
Please go to Settings > Sources > ${this.baseUrl} and press Cloudflare Bypass`
  ...
  var KaliScanParser = class extends BuddyComplexParser {
  ...
  var DOMAIN = "https://kaliscan.io";
  var KaliScanInfo = {
    version: getExportVersion("0.0.2"),
    name: "KaliScan",
    description: `Extension that pulls manga from ${DOMAIN}`,
    author: "Benjamin",
    authorWebsite: "https://github.com/benjamin5896",
    icon: "icon.png",
  ...
  var KaliScan = class extends BuddyComplex {
    constructor() {
      super(...arguments);
      this.baseUrl = DOMAIN;
      this.parser = new KaliScanParser();
    }
    async getHomePageSections(sectionCallback) {
    ...
    async getChapters(mangaId) {
      ...
      return this.parser.parseChapterList(mangaPage$, mangaId);
    }
    async getNumericMangaId(mangaId) {
      ...
      const correctScriptMatches = String(rawData).match(/var\s+bookId\s*=\s*(\d+)/);
      if (correctScriptMatches?.[1]) return correctScriptMatches[1];
      throw new Error("Failed to get numeric manga id");
    }
  };
  return __toCommonJS(KaliScan_exports);
})();
